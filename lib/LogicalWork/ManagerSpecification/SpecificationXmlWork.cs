...
namespace LogicalWork.ManagerSpecification
{
    ...
    public sealed class SpecificationXmlWork : IWork
    {
        ...
        {
            ResultSpecification resultSpecification = new ResultSpecification { NumberOrder = order.Name };

            //Получили коллекцию негабаритных частей заказа
            IEnumerable<Item> notDimensionEnumerator = items.Where(predicate: item => item.Weight >= this.MaxWeight);
            ...
            IEnumerable<Item> bigContainer = items.Where(predicate: item => item.Weight > this.MaxWeight - minWeight);
        ...
        private uint AlgoritmContainer(IEnumerable<Item> items)
        {
            ...
            {
                if (conteiterWeight < this.MaxWeight - item.Weight)
                {
